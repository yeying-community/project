...
> 历史文档：本文记录早期把 Node 作为 AppStore 运行入口的设计，不再作为当前架构依据。当前职责边界见 [应用运行时职责边界](../../架构/应用运行时职责边界.md)，字段契约见 [智能体运行时协议 v1](../../协议/智能体运行时协议-v1.md)：Project 只消费 Agent Runtime，Agent 负责安装、升级、失败回滚和卸载，Node 只做社区发布目录与授权入口。
